...
public static class HouseMaskEnumerable
{
	...
	extension(HouseMask @this)
	{
		...
		public ReadOnlySpan<T> Select<T>(Func<House, T> selector)
		{
		...
		public HouseMask Where(Func<House, bool> predicate)
		{
			var result = 0;
		...
		/// <paramref name="collectionSelector"/> on each element of the current instance and then mapping each of those sequences
		/// and their corresponding source element to a result element.
		/// </returns>
		public ReadOnlySpan<TResult> SelectMany<TResult>(
			Func<House, CellMap> collectionSelector,
			Func<House, Cell, TResult> resultSelector
		)
		{
			var result = new List<TResult>(PopCount((uint)@this) << 1);
			foreach (var digit in @this)
			{
