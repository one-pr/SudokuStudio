...
public static class CantorExpansion
{
	...
	public static int RankRelabeledDigits(ReadOnlySpan<Digit> digits)
	{
		...
		var n = digits.Length;
		ArgumentException.Assert(baseOrder.Length == n);
		...
		for (var i = 0; i < n; i++)
		{
			...
			InvalidOperationException.Assert(j != -1);
	...
	{
		ArgumentException.Assert(lines.Length == 9);
	...
	{
		ArgumentException.Assert(perm.Length == 3);
