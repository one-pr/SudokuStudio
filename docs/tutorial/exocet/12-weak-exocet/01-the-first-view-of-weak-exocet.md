...
第三部分的删数来自于伤口。伤口 `r6c5` 和强心针单元格 `r4c8` 同处于一个大行 `mr2` 上，另外伤口所在的大列是 `mc2`。我们按交叉单元格所分布的这几个宫作基础，刚才的这大行和大列会构成“L”或“T”字母形状的连线。这个“L”或“T”具体看的是这个拐弯的是在中间还是在边上，在边上就是 L 形，在中间就是 T 形；两者并无使用和理解上的差异，只是不太好用一个字母表达出两种摆放。然后，其中形状里连线的短边到 `b8`，那么这部分的删数就来自于 `b8`，和保持和伤口和强心针单元格连线方向平行的大行 `mr3` 上的 `b7` 这个宫。
